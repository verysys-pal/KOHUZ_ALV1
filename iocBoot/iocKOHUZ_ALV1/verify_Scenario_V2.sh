...
# ───────────────────────────────────────────────────────────────────
echo ""
echo "━━━ [Step 7] 소프트 리미트 위반 테스트 (.HLM / .LLM / .LVIO) ━━━"

# 리미트 값 읽기
HLM_VAL=$(safe_caget "${MOTOR_PV}.HLM") || HLM_VAL=""
LLM_VAL=$(safe_caget "${MOTOR_PV}.LLM") || LLM_VAL=""

if is_number "$HLM_VAL" && is_number "$LLM_VAL"; then
    
    # [내부 함수] 리미트 테스트 로직
    # check_limit "방향라벨" "리미트값" "초과오프셋"
    check_limit() {
        local dir_label="$1"
        local limit_val="$2"
        local offset="$3"
        ...
        echo "  ──────────────────────────────────────────"
        echo "  [Test] ${dir_label} 방향 리미트 테스트 (Limit: ${limit_val})"

        # 1. 원점 이동 (Start from Origin)
        echo "  Action: 원점(0)으로 이동"
        caput "${MOTOR_PV}.VAL" 0 > /dev/null 2>&1
        if ! wait_for_done 30; then
            record_result "Step7_Limit_${dir_label}" "FAIL" "이동 실패(원점)"
            return
        fi

        # 2. 리미트 근접 이동 (Safe Move)
        local target_safe
        target_safe=$(python3 -c "print($limit_val - ($offset))")
        echo "  [1차 이동] 리미트 근접 위치로 이동: ${target_safe}"
        
        caput "${MOTOR_PV}.VAL" "$target_safe" > /dev/null 2>&1
        if ! wait_for_done 30; then
            echo "  ⚠️  리미트 근접 이동 실패 (Safe Target=${target_safe})"
            # 실패해도 위반 테스트 진행 (혹은 중단?) -> 진행
        else
            echo "  ✅ 리미트 근접 이동 완료"
        fi

        # 3. 초과 목표 설정 (Violation Move)
        local target_over
        target_over=$(python3 -c "print($limit_val + ($offset))")
        echo "  [2차 이동] 리미트 초과 시도: ${target_over} (Offset: ${offset})"

        caput "${MOTOR_PV}.VAL" "$target_over" > /dev/null 2>&1
        sleep 2.0 # Check interval

        # 4. LVIO 상태 확인
        local lvio_val
        lvio_val=$(safe_caget "${MOTOR_PV}.LVIO") || lvio_val="N/A"

        if [ "$lvio_val" = "1" ]; then
            echo "  ✅ LVIO=1 정상 검출 (${dir_label})"
            record_result "Step7_Limit_${dir_label}" "PASS" "LVIO=1 정상 (Safe=${target_safe} -> Over=${target_over})"
        else
            echo "  ❌ LVIO 미검출 (LVIO=${lvio_val})"
            print_msta_detail
            record_result "Step7_Limit_${dir_label}" "FAIL" "LVIO 미검출"
        fi
    ...
    # 양방향 테스트 실행 (35mm 근처로 예상되므로 offset 1.0 사용 -> 34mm이동 후 36mm시도)
    check_limit "Positive" "$HLM_VAL" "1.0"
    check_limit "Negative" "$LLM_VAL" "-1.0"

else
    record_result "Step7_SoftLimit" "SKIP" "HLM/LLM 값 읽기 실패"
fi
...
# H/W 리미트 테스트 함수
# check_hw_limit "방향라벨" "이동명령(JOGF/JOGR)" "리미트PV(HLS/LLS)" "SafeVal"
check_hw_limit() {
    local dir_label="$1"
    local jog_cmd="$2"
    local limit_pv="$3"
    local limit_pv="$3"
    # local safe_target="$4"  <-- Removed unused argument to fix unbound variable error

    echo "  ──────────────────────────────────────────"
    echo "  [Test] ${dir_label} 방향 하드웨어 리미트 테스트"
    ...
    # 2. 리미트 향해 조그 이동
    echo "  Action: ${dir_label} 방향으로 리미트까지 이동 (${jog_cmd}=1)"
    caput "${MOTOR_PV}.${jog_cmd}" 1 > /dev/null 2>&1
    
    # 3. 리미트 감지 대기 (최대 30초)
    local hit_limit=false
    for ((i=0; i<60; i++)); do
        local ls_val
        ls_val=$(safe_caget "${MOTOR_PV}.${limit_pv}") || ls_val=""
        if [ "$ls_val" = "1" ]; then
            echo "  ✅ ${limit_pv} 리미트 감지됨! (Loop $i)"
            hit_limit=true
            break
        fi
        sleep 0.5
    ...
    if [ "$hit_limit" = true ]; then
        record_result "Step8_HWLimit_${dir_label}" "PASS" "${limit_pv} 동작 확인"
    else
        echo "  ❌ ${limit_pv} 감지 실패 (Timeout)"
        record_result "Step8_HWLimit_${dir_label}" "FAIL" "${limit_pv} 미감지"
    fi

    # 5. 복구 (리미트 해제 방향으로 소폭 이동 후 원점 복귀)
    echo "  → 리미트 해제 위해 반대 방향 이동"
    # 간단히 원점 복귀 시도
    caput "${MOTOR_PV}.VAL" 0 > /dev/null 2>&1
    wait_for_done 30 || true
}

# 양방향 테스트 (주의: 실제 장비 충돌 가능성 확인 필요)
# ※ 안전을 위해 주석 처리하거나, 작업자의 명시적 요청 시에만 수행하도록 구성 권장
# 여기서는 지시사항에 따라 구현함.

# (1) Positive Limit (HLS)
check_hw_limit "Positive" "JOGF" "HLS" 
# (2) Negative Limit (LLS)
check_hw_limit "Negative" "JOGR" "LLS" 

# 최종 상태 확인
HLS=$(safe_caget "${MOTOR_PV}.HLS") || HLS="N/A"
LLS=$(safe_caget "${MOTOR_PV}.LLS") || LLS="N/A"
echo "  [최종 상태] HLS=$HLS, LLS=$LLS"
